...
import {wxPaymentApplication, aliWapPaymentApplication,activityBuyGoods,orderStatus} from '@/api'
  ...
  methods: {
    async creatOrder() {
      ...
      try {
        ...
        let params = {
          activityId: this.grouponInfo.id,
        }
        const {resultData} = await activityBuyGoods(params);
      ...
      setTimeout(async ()=>{
        ...
        if (resultData && resultData.status===0) {
          ...
          if (type===1) {
            ...
            this.$router.push({path:'/result',query:{fail:'1',fromPage:'activity'}});
          }
        }
      },1000)
    },
