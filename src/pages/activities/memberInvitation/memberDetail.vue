<template>
  <div class="invite">
    <div class="invite_head" v-if="showButton && brandPromotionInfo.topImg">
      <img class="head_img" :src="brandPromotionInfo.topImg"/>
      <img v-if="!brandPromotionInfo.userImg || brandPromotionInfo.userImg==='default.jpg'"
           src="@/assets/img/memberInvitation/default-img.png"
           class="avatar"
           alt=""/>
      <img v-else
           class="avatar"
           :src="brandPromotionInfo.userImg"
           alt=""/>
      <span class="name">{{brandPromotionInfo.userName}}</span>
    </div>
...
export default {
  ...
  created() {
    this.form.memberActivityCode = decodeURIComponent(this.$route.query.id);
  ...
  methods: {
    ...
    async getActivityDetail() {
      try {
        const {resultData} = await getActivityDetail({code:decodeURIComponent(this.$route.query.id)});
        this.showButton = true;
        this.brandPromotionInfo = resultData;
        this.brandPromotionInfo.code = decodeURIComponent(this.$route.query.id);
        this.form.code = resultData.recommendCode;
        this.form.recommendCode = resultData.followUser;
        await this.getBase64ImgCode();
      } catch (e) {
        // showRegister
        if (e.resultMessage.indexOf("结束") < -1 || e.resultMessage.indexOf("下架") < -1) {
    ...
    async login() {
      ...
      try {
        ...
        this.brandPromotionInfo.token = token;
        ...
        setTimeout(() => {
          ...
          if (this.showRegister) {
            ...
            // };
            Cookies.set('memberInviteInfo', JSON.stringify(this.brandPromotionInfo));
            this.$store.dispatch("content/setGroupData", this.brandPromotionInfo).then(() => {
              this.$router.push({path: "/memberOrder", query: {isH5: 'y'}});
            });
          }
        }, 2000);
      } catch (e) {
